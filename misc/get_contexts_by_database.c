...
cson_value *get_contexts_by_database(struct MHD_Connection *conn, char* error, size_t sz)
{
    const char *q, *row;
    cdb2_hndl_tp *hndl;
    int rc, i;
    ...
    rc = cdb2_open(&hndl, "comdb2perfdb", "localhost", CDB2_DIRECT_CPU);
    if (rc != 0)
        return NULL;

    snprintf(query, sizeof(query),
             "SELECT DISTINCT(context) FROM contexts WHERE dbname LIKE '%s'",
             q);
...
out:
    ...
    if (rc != 0 && rc != CDB2_OK_DONE) {
        strncpy(error, cdb2_errstr(hndl), sz);
